...
- **WebDAV 同步 effort(2026-09-14 立项,尚未开工)**:决议已逐题裁定并汇编为 [.scratch/confidant-sync/spec.md](.scratch/confidant-sync/spec.md)(含调研依据与待验证清单);**实施票已拆为 8 张**,在 [.scratch/confidant-sync/issues/](.scratch/confidant-sync/issues/)(票头 `Status:`/`Blocked by:`,01→02→03 主干,04–07 并行,08 真机验收收口);范围变更与两条架构抉择记在 [docs/adr/](docs/adr/) 0001–0003。口径要点:文件级双向镜像、手动触发、远端保持纯文件夹、明文存储、串行请求;「绝不静默丢内容」是该方案全部保护措施的来源。
