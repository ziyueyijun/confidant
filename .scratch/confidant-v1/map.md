...
- Typora 候选功能面的逐项进/出仲裁:表格、图片粘贴与存放、全文搜索、主题与外观自定义、大纲/侧边面板、数学公式、脚注/TOC、拼写检查、字数统计等 → 待「首版用户画像」与「范围裁决标准」落定后,才能按主题拆票。
- 编辑引擎选型的最终决策与验证:研究票会给候选池,但"选谁、怎么验证"是 HITL 决策,需原型验证票 → 待「编辑引擎现状研究」。
- 编辑面深层交互(块级操作、光标语义、快捷键集):依赖引擎选型结果。
