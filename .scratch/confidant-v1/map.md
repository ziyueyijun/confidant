...
- [图片粘贴与存放仲裁](issues/11-image-handling-arbitration.md):v1 图片 = 粘贴/拖入自动落盘同目录(相对引用、`{笔记名}-{时间戳}` 命名、零配置)+ Chromium 原生格式渲染(含 GIF)+ 右键菜单(打开所在文件夹/复制/删除进回收站);缩放对齐图注/图床/网络图/批量管理 = 出(图床触本地优先一票否决);粘贴落盘入实现首验。
- [应用壳仲裁](issues/15-app-shell-arbitration.md):v1 壳 = 菜单栏(命令可发现性通道,含深浅切换「跟随系统/浅/深」)+ 启动恢复上次工作区(首启走欢迎页);设置面板/托盘/状态栏/多窗口/菜单自动隐藏 = 出或延后;自动更新移交实现 effort。
- [编辑面深层交互](issues/13-editing-interactions.md):自动保存(停顿防抖 + Ctrl+S,原子写盘);外部删除提示条不静默重建;Enter 分段 / Shift+Enter 硬换行;格式入口 = 选中浮动工具条 + 段落/格式菜单(10 的 H5 在此进);粘贴富文本语义转/纯文本原样;表格右键增删行列 + Tab/Enter;任务点击勾选 + Tab 缩进;撤销重做;固定快捷键集(自定义键位出);输入即转(语法驱动)= 出。
- [导出格式与管线裁决](issues/14-export-arbitration.md):v1 导出 = 文件菜单「导出 PDF/打印」(渲染通道直接成 PDF,恒浅色观感,默认 A4);同目录同名即覆盖(生成物非数据);成功提示 + 打开所在文件夹;Pandoc 全家桶/导出设置 = 出;HTML 长图/docx = 延后;导出不动源文件。
- [收官补裁](issues/16-closing-gaps.md):规格汇编暴露的三处漏裁——文件列表面板(Articles)= 延后;树操作撤销(移动/重命名后单步)= 进;链接交互 = 单击编辑不跳、右键/Ctrl+单击打开(http 走浏览器、工作区内 md 应用内开、其余资源管理器显示)。

<!-- 目的地达成:全部 16 张票关闭,frontier 与 fog 皆空。目的地落盘点已执行——首版规格汇编完毕,见同目录 spec.md。 -->

## Not yet specified

<!-- 在途 fog:朝向目的地、尚不足以精确成票的部分;frontier 推进后逐块毕业成票,毕业一块清一块。 -->

(空。最后一缕 fog——应用壳——已毕业为 15 应用壳仲裁;frontier 全清后即到达目的地,进入规格落盘。)
